package
{
	...
	import starlingOrigin.textures.TextureAtlas;
	
	public class MapToolResources extends EventDispatcher
	{	
		...
		public function MapToolResources(spriteDirectory:File = null, soundDirectory:File = null, imageDirectory:File = null)
		{
		...
		public function loadResource():void
		{
			...
			if(_spriteDir != null)
			{
				var spriteLoader:MapToolSpriteLoader;
				for(var i:int = 0; i < _spriteName.length; i++)
				{
					spriteLoader = new MapToolSpriteLoader(onCompleteSpriteLoad, onFaildSpriteLoad);
		...
		private function onImageLoadedFailed(event:IOErrorEvent):void
		{
			...
			dispatchEvent(new MapToolLoadingEvent(MapToolLoadingEvent.FAILED, event.text));
		}
		...
		{
			dispatchEvent(new MapToolLoadingEvent(MapToolLoadingEvent.FAILED, message));
		}
		
		private function checkLoadComplete(fileName:String):void
		{
			dispatchEvent(new MapToolLoadingEvent(MapToolLoadingEvent.PROGRESS, _loadedCount));
			...
			if(_loadedCount >= (_spriteName.length + _soundName.length + _imageName.length))
			{
				...
				dispatchEvent(new MapToolLoadingEvent(MapToolLoadingEvent.COMPLETE));
			}
		}
